...
export default function TasksBoard({
  ...
  const [creating, setCreating] = useState<TaskStatus | false>(false);
...
function NewTaskModal({
  ...
  const selectedBrand = brandId ? brands.find((b) => b.id === brandId) ?? null : null;
  ...
  const dueLabel = dueDate ? dueDateLabel(new Date(dueDate).toISOString()) : null;
  const prio = PRIORITY_META[priority];
  const COLUMN_META = useColumnMeta();
  const { labelFor } = useColumnsList();
  const metaFallback = COLUMN_META[Object.keys(COLUMN_META)[0]];
  const colMeta = COLUMN_META[initialStatus] ?? metaFallback;
  const completedSubsCount = 0; // En el modal de creación no hay subtasks completadas todavía
  // Status del modal — permite cambiarlo antes de crear (no solo el initial)
  const [status, setStatus] = useState<TaskStatus>(initialStatus);
  const statusMeta = COLUMN_META[status] ?? metaFallback;
    ...
    try {
      const res = await fetch("/api/tasks", {
        ...
        body: JSON.stringify({
          ...
          // Mando el primero como assigneeId (compat con el endpoint POST que
          // todavía espera assigneeId single). El resto se agrega abajo via
          // PUT /assignees.
      ...
      const j = await res.json();
      // Si hay más de 1 assignee, mando el set completo a /assignees
      ...
      // Asignar tags si hay alguna seleccionada (es un PUT separado)
      if (tagIds.length > 0) {
        await fetch(`/api/tasks/${j.task.id}/tags`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ tagIds }),
        }).catch(() => null);
  ...
  return (
    ...
    >
      <div
        className="card animate-task-card-in flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden p-0 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header con chips de status + prioridad + marca (igual que el drawer) */}
        <header className="relative flex items-center justify-between gap-3 border-b divider px-5 py-3.5">
          {/* Acento de color del estado a la izquierda */}
          <span
            aria-hidden
            className={`absolute left-0 top-0 h-full w-1 ${statusMeta.pill}`}
          />
          <div className="flex flex-1 flex-wrap items-center gap-1.5 pl-2">
            <span
              className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10.5px] font-bold uppercase tracking-wider text-white shadow-sm ${statusMeta.pill}`}
            >
              {labelFor(status)}
            </span>
            ...
            {selectedBrand && (
              <span
                ...
                style={{
                  background: `${selectedBrand.color ?? "#a1a1aa"}1a`,
                  color: isDarkHex(selectedBrand.color ?? "")
                    ? "#3f3f46"
                    : selectedBrand.color ?? "#71717a",
                }}
              >
                <span
                  className="inline-block h-1.5 w-1.5 rounded-full"
                  style={{ background: selectedBrand.color ?? "#a1a1aa" }}
                />
                {selectedBrand.name}
              </span>
            )}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          {/* Title — input grande estilo drawer */}
          <input
            autoFocus
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="¿Qué hay que hacer?"
            className="-mx-2 w-[calc(100%+1rem)] rounded-lg border border-dashed border-zinc-200 bg-zinc-50/60 px-2 py-1.5 text-[22px] font-bold tracking-tight text-zinc-900 transition placeholder:text-zinc-400 hover:border-zinc-300 hover:bg-zinc-50 focus:border-solid focus:border-fuchsia-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-fuchsia-100"
            onKeyDown={(e) => {
              if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) submit();
            }}
          />

          {/* Properties — pickers consistentes con el drawer */}
          <div className="mt-5 space-y-1.5">
              ...
              >
                {selectedAssignees.length === 0 ? (
                  ...
                  <span className="flex items-center gap-2">
                    <Avatar user={selectedAssignees[0]} size="md" ring />
                    <span className="text-[13px] font-medium text-zinc-800">
                      {selectedAssignees[0].id === currentUserId
                        ? "Yo"
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] font-medium text-zinc-800">
                      {selectedAssignees.length} personas
                    </span>
                  </span>
                )}
              </PropertyPicker>
            ...
            {/* Estado (permite cambiarlo antes de crear) */}
            <PropertyRow label="Estado" icon={CheckCircle2}>
              ...
              >
                <span className="flex items-center gap-2">
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[10.5px] font-bold uppercase tracking-wide text-white ${statusMeta.pill}`}
                  >
                    {labelFor(status)}
                  </span>
                </span>
          ...
          {/* Description con TipTap (mismo que el drawer) */}
          <div className="mt-5">
            <DescriptionEditor
              value={description}
              canWrite
            ...
            {subtaskInputs.length > 0 && (
              <ul className="mb-2 space-y-0.5">
                {subtaskInputs.map((s, i) => (
                  <li
                    key={i}
                    className="group/sub flex items-center gap-1.5 rounded-lg px-1.5 py-1.5 transition hover:bg-zinc-50"
                  >
        ...
        <div className="flex items-center justify-end gap-2 border-t divider bg-white px-5 py-3">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg px-3 py-1.5 text-sm font-semibold text-zinc-600 transition hover:bg-zinc-100"
          >
            Cancelar
          </button>
          <button
            ...
            className="btn-gradient inline-flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-sm font-semibold disabled:opacity-50"
          >
